    ...
    <table class="table table-hover align-middle mb-0 text-nowrap" style="min-width: max-content;">
        <thead class="table-light">
            <tr>
                ...
                <th>Date & Time</th>
                <th>Error Log</th>
                <th class="text-center pe-4">Error Report</th>
            </tr>
        </thead>
        <tbody>
            @forelse($tracks ?? [] as $index => $track)
                <tr>
                    ...
                    <td>
                        @if ($track->error_log)
                            <button class="btn btn-sm btn-light text-danger shadow-sm border btn-view-logs"
                                data-log="{{ $track->error_log }}"
                                title="View Errors">
                                <i class="fas fa-exclamation-triangle me-1"></i> View Logs
                            </button>
                        @else
                    ...
                    <td class="text-center pe-4">
                        @if ($track->error_file_path)
                            <a href="{{ route('users.uploads.errors', $track->id) }}" class="btn btn-sm btn-outline-danger shadow-sm border px-3"
                ...
                <tr>
                    <td colspan="9" class="text-center py-5 text-muted">
                        <div class="d-flex flex-column align-items-center">
                            ...
                            <p class="mb-0">You haven't uploaded any user files yet.</p>
                        </div>
                    </td>
                </tr>
            @endforelse
