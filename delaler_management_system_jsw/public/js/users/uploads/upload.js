...
$(document).ready(function () {
    ...
    const fetchUploads = async (page = 1, searchQuery = "") => {
        try {
            ...
            let response = await apiRequest.formPost(
                formData,
                "/users/uploads/list",
                false, // don't show full page loading overlay, refresh quietly
            );
            ...
            if (response.success) {
                ...
                if (response.html.includes("fa-spinner fa-spin") || response.html.includes("Pending")) {
                    startPolling();
                } else {
                    stopPolling();
                }
    ...
    // 2. Upload Form Logic
    // ==========================================
    $("#uploadUserForm").on("submit", async function (e) {
        e.preventDefault();
        ...
        try {
            ...
            console.error("Error submitting user upload:", error);
            Swal.fire({
                icon: "error",
                title: "Error",
                text: "Failed to upload file due to network or server issues.",
            });
        } finally {
