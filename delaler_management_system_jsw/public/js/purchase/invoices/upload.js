$(document).ready(function() {
    ...
    try {
        ...
        console.error('Error during initial load of purchase invoice upload tracks:', err);
    }
    ...
    $('#btnRefreshHistory').on('click', function() {
        ...
        try {
            ...
            console.error('Error refreshing invoice upload history:', err);
            btn.prop('disabled', false);
            icon.removeClass('fa-spin');
        }
    });
    ...
    function loadUploadTracks(page = 1, search = '', callback = null) {
        try {
            let listUrl = window.PurchaseInvoiceUploadConfig ? window.PurchaseInvoiceUploadConfig.listUrl : '';
            let csrfToken = window.PurchaseInvoiceUploadConfig ? window.PurchaseInvoiceUploadConfig.csrfToken : '';
    ...
    $('#purchaseInvoiceUploadForm').on('submit', function(e) {
        try {
            ...
            let btn = $('#btnSubmit');
            let importUrl = window.PurchaseInvoiceUploadConfig ? window.PurchaseInvoiceUploadConfig.importUrl : '';
                ...
                success: function(response) {
                    try {
                        ...
                        if (response.success) {
                            ...
                            $('#purchaseInvoiceUploadForm')[0].reset();
                            setTimeout(function() {
                                try {
                                    $('#uploadPurchaseInvoiceModal').modal('hide');
            ...
            console.error('Error in purchaseInvoiceUploadForm submit handler:', err);
