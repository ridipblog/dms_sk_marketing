...
$(document).ready(function () {
    const fetchReport = async (page = 1) => {
        ...
        try {
            ...
            let response = await apiRequest.formPost(
                formData,
                "/reports/dealer-wise-sales/list",
                false,
            );
            ...
            if (response?.success ?? null) {
                ...
                reusebase.showSwal(
                    "error",
                    "Error",
                    response?.message || "Failed to fetch dealer sales report",
                );
    ...
    $("#btnExportReport").on("click", function (e) {
        ...
        window.location.href = `/reports/dealer-wise-sales/export?${params.toString()}`;
    });
});
